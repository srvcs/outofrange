use srvcs_outofrange::{api::Deps, config::Config, health, router, telemetry};
...
async fn main() {
    ...
    tracing::info!(addr = %cfg.bind_addr, env = %cfg.environment, "srvcs-outofrange listening");
    let deps = Deps {
        between_url: cfg.between_url.clone(),
